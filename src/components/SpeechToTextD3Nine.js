...
function SpeechToText(props) {
    ...
    let pretendSAF = randomArray(SplitSize, maxPretendNeme);
    console.log('SAF',pretendSAF);

    
    // turn random Neme into an object

    let randoNemeObject = pretendSAF.map( s => ({SAF:s}) );
    console.log ('randoNemeObject', randoNemeObject);

    
    // turn text result into object
    
    let objectText = SplitTextResult.map( s => ({Text:s}) );
    console.log ('Text', Text);

    // merge

    let stuff = _.merge(randoNemeObject, objectText);
    console.log('mergedObjects', stuff);
    ...
    let mergedLength = (Object.keys(mergedObjects).length)/2;
